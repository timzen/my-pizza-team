/**
 * WorkflowEditor — Editing controls for a workflow's states, transitions,
 * and categories. Manages local editing state and saves via PUT /api/config.
 *
 * Ported from the old WorkflowCard component in ConfigPage.tsx, adapted
 * for the standalone workflow detail page.
 */
...
export function WorkflowEditor({ name, workflow, config, isDefault, onSaved }: Props) {
  ...
  const toggleCategory = (cat: string) => {
    const wfCats = new Set(wf.categories || []);
    if (wfCats.has(cat)) wfCats.delete(cat);
    else wfCats.add(cat);
    setWf({ ...wf, categories: wfCats.size > 0 ? [...wfCats] : undefined });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {allCategories.length > 0 && (
        <Card>
          <CardContent className="p-4 space-y-3">
            <h3 className="text-sm font-semibold">Default Memory Categories</h3>
            <p className="text-xs text-muted-foreground">
              Stories using this workflow inherit these categories unless overridden.
            </p>
            <div className="flex gap-1 flex-wrap">
              {allCategories.map((c) => {
                const selected = (wf.categories || []).includes(c);
                return (
                  <Button
                    key={c}
