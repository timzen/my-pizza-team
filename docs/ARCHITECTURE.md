...
- **ui/** — Frontend application (React + Vite + shadcn/ui). Talks to the daemon's HTTP API.
  - `src/App.tsx` — Router + shell layout: `NavBar` on top, a scrollable `<main>` for the routed page, and a persistent `TeammateSidebar` on the right (shown on `lg+`). Pages: `/` + `/context` (RootPage — Workflows/Context tabs), `/board`, `/scratchpad`, `/assistant`, `/task/:storyId/:taskId`, `/story/:id`, `/backlog`, `/archived`, `/config` (+ `/config/:tab`), `/workflows/:name`, `/help`. Route-driven tab groups (RootPage's Workflows/Context, the Board's Board/Backlog/Archive, ConfigPage's General/Teammates/Capabilities) share `src/components/RouteTabs.tsx`.
