...
- **ui/** — Frontend application (React + Vite + shadcn/ui). Talks to the daemon's HTTP API.
  ...
  - `src/components/dock/SideDock.tsx` — **The left dock**: a tab bar (**Assistant** with an unread badge · **Team** with the online count and an amber attention dot for at-risk work or a pool that can't spawn), both tab bodies kept mounted (a draft survives a peek at the team), quick-create under both, drag-to-resize (300–560px), a collapsed icon rail (chat, quick-create, team size/spawn, agent avatars linking to their live view, queue count), and a floating corner panel below `lg`. It owns the chat stream (`useAssistantStream`) and team data (`src/hooks/useTeamData.ts`) so badges stay live, plus the team-size/spawn dialogs so the rail can open them. `src/components/dock/SideDockProvider.tsx` + `src/hooks/useSideDock.ts` hold open/tab state (localStorage); `OpenAssistantTab` backs the `/assistant` redirect.
  - `src/components/team/TeamPanel.tsx` — The **Team** tab: a toolbar (online count, team size, spawn), the **teammates** — never the leader, which is the agent behind the Assistant tab (filtered once in `useTeamData`, so the tab badge, toolbar count, and rail avatars all agree) — (they link to `/teammates/:id` and stay highlighted while it's in the center; pending spawns; offline), and the **Queue** (non-terminal WorkItems — READY/IN_PROGRESS/MORIBUND — with recovery actions: cancel a READY item, force-fail a MORIBUND one, or force-fail-and-re-enqueue). Rows/avatars/buttons are in `src/components/team/TeamParts.tsx`; shared types + `roleOf`/`viewPath` in `src/lib/team.ts`. Where an always-visible queue should live is open (DESIGN.md).
