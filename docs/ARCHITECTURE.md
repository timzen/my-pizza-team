...
- **ui/** — Frontend application (React + Vite + shadcn/ui). Talks to the daemon's HTTP API.
  ...
  - `src/pages/TasksPage.tsx` / `src/pages/SchedulePage.tsx` — Standalone WorkDefs: **Solitary** one-shots (Tasks) and **Scheduled** cron jobs (Schedule). Both list `/api/work-defs` (filtered by type) with a **Run**/Run-now action; Schedule shows a friendly cron description. `src/pages/NewWorkDefPage.tsx` is the shared create form (`/work-defs/new?type=…` — the type is fixed by the query param, no in-UI switch; Scheduled adds cron presets, Solitary an "enqueue now" toggle). Acceptance criteria are entered as an add-as-you-go list (`src/components/ui/acceptance-criteria-editor.tsx`) that round-trips a markdown bullet list and scores each item against **RFC 2119** (normative / recommended / optional / vague badge). `src/pages/WorkDefDetailPage.tsx` is the view/edit page with the per-def **run thread** (comments — including agent completion summaries).
